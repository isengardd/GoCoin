	...
	if this.HasPosition() {
		...
		if buycount > 0.2 {
			...
			if orderId != 0 {
				...
				rows, err := coinapi.GetDB().Query(fmt.Sprintf("INSERT INTO order_record(coin_type,order_id,order_type,order_time,order_time_sell,low_price,high_price) VALUES('%s', %d, '%s', NOW(), NOW(), %f, %f)",
